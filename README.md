# Dreame Cloud for Home Assistant
...
- State attributes: cleaning time, area, error code, room list

### Map Camera
- Live map rendering from decoded Dreame map data
- Color-coded rooms, walls, and carpet zones
- Robot and charger position indicators
...
### Controls
- **Selects**: Suction level, water volume, cleaning mode
...
4. Search for "Dreame Cloud" and install
...
- Python 3.13+ (required by dreame-mocker)
...
Config Flow → DreameCloud.connect() → auth + device discovery
     ↓
Coordinator (30s poll) → DreameDevice.get_status()
     ↓                 → DreameDevice.get_properties() (consumables)
     ↓                 → DreameDevice.get_map() (periodic)
     ↓
Entities ← DreameCloudData (status + map + consumables)
```
