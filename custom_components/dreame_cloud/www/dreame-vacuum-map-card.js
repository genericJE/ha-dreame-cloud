(() => {
  ...
  __name(batteryIcon, "batteryIcon");

  // src/constants.js
  var SUCTION_MAP = { Quiet: 0, Standard: 1, Strong: 2, Turbo: 3 };
  var WATER_MAP = { Low: 1, Medium: 2, High: 3 };
  var CLEANING_MODE_MAP = { Sweeping: 0, Mopping: 1, "Sweep & Mop": 2 };
  var SUCTION_OPTIONS = ["Quiet", "Standard", "Strong", "Turbo"];
  var WATER_OPTIONS = ["Low", "Medium", "High"];
  var CLEANING_MODE_OPTIONS = ["Sweeping", "Mopping", "Sweep & Mop"];
  var RECT_TOOL_COLORS = {
    no_go: "#f44336",
    carpet: "#9c27b0",
    low_clearance: "#2196f3",
    ramp: "#ff9800"
  };
  var LINE_TOOL_COLORS = {
    wall: "#f44336",
    threshold: "#4caf50",
    impassable: "#ff5722",
    cliff: "#795548"
  };
  var RECT_TOOLS = ["no_go", "carpet", "low_clearance", "ramp"];
  var LINE_TOOLS = ["wall", "threshold", "impassable", "cliff"];
  var FURNITURE_TYPES = {
    1: { name: "Single Bed", w: 2e3, h: 1e3 },
    2: { name: "Double Bed", w: 2e3, h: 1600 },
    3: { name: "Armchair", w: 800, h: 800 },
    4: { name: "Two Seat Sofa", w: 1400, h: 800 },
    5: { name: "Three Seat Sofa", w: 2e3, h: 800 },
    6: { name: "Dining Table", w: 1200, h: 800 },
    7: { name: "Nightstand", w: 500, h: 500 },
    8: { name: "Coffee Table", w: 1e3, h: 500 },
  ...
  // src/editor.js
  var DreameVacuumMapCardEditor = class extends HTMLElement {
    static {
      __name(this, "DreameVacuumMapCardEditor");
    }
    constructor() {
      super();
    ...
    setConfig(config) {
      const needsRender = !this._rendered || config.entity !== this._config.entity || config.map_entity !== this._config.map_entity;
      this._config = { ...config };
    ...
    _getVacuumEntities() {
      ...
      return Object.keys(this._hass.states).filter((id) => id.startsWith("vacuum.")).map((id) => ({
        id,
    ...
    _render() {
      ...
      const roomAliasesHtml = roomEntries.length > 0 ? roomEntries.map(([segId, room]) => {
        ...
        return `
          ...
          `;
      }).join("") : '<div class="hint">No rooms detected yet. Rooms will appear here after the map loads.</div>';
    ...
    `;
      ...
      this.shadowRoot.querySelectorAll(".alias-input").forEach((input) => {
        input.addEventListener("input", (e) => {
          ...
          const val = e.target.value.trim();
          const aliases2 = { ...this._config.room_aliases || {} };
          if (val) {
            aliases2[segId] = val;
          } else {
            delete aliases2[segId];
          }
          this._config = { ...this._config, room_aliases: aliases2 };
          fireEvent(this, "config-changed", { config: this._config });
        });
      });
  ...
  var DreameVacuumMapCard = class _DreameVacuumMapCard extends HTMLElement {
    static {
      __name(this, "DreameVacuumMapCard");
    }
    constructor() {
      super();
      this.attachShadow({ mode: "open" });
      this._config = {};
      this._hass = null;
      this._selectedRooms = /* @__PURE__ */ new Set();
      this._roomOrder = [];
      this._mode = _DreameVacuumMapCard._persistedMode || "all";
      this._roomView = "map";
    ...
    `;
      ...
      card.querySelectorAll(".tab").forEach((tab) => {
        tab.addEventListener("click", () => {
          ...
          this._mode = newMode;
          _DreameVacuumMapCard._persistedMode = newMode;
    ...
    _updateHeader(card) {
      ...
      if (isActive) {
        ...
        const area = cleaningArea ? `${cleaningArea.state} m\xB2` : "";
        statsHtml = `
        <div class="stats">
      ...
      header.querySelectorAll(".header-icon-btn").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          if (this._mode === newMode) {
            this._mode = "all";
            _DreameVacuumMapCard._persistedMode = "all";
    ...
    _updateMap(card) {
      ...
      for (const [editZones, attrKey, fill, stroke, toolId] of _zoneStyles) {
        ...
        for (let zi = 0; zi < zones.length; zi++) {
          const zone2 = zones[zi];
          if (!zone2.points || zone2.points.length !== 4) continue;
          const isSelectedZone = this._mode === "edit" && this._editTool === toolId && zi === this._selectedZoneIdx;
          const pts = zone2.points.map((p) => `${p.x},${p.y}`).join(" ");
          svgContent += `
          <polygon points="${pts}"
      ...
      const passableThresholds = this._mode === "edit" ? this._editPassableThresholds : camera.attributes.passable_thresholds || [];
      ...
      const impassableThresholds = this._mode === "edit" ? this._editImpassableThresholds : camera.attributes.impassable_thresholds || [];
      ...
      const cliffs = this._mode === "edit" ? this._editCliffs : camera.attributes.cliffs || [];
      ...
      const ramps = this._mode === "edit" ? this._editRamps : camera.attributes.ramps || [];
      for (let ri = 0; ri < ramps.length; ri++) {
        const ramp = ramps[ri];
        if (!ramp.points || ramp.points.length !== 4) continue;
        const isSelectedRamp = this._mode === "edit" && this._editTool === "ramp" && ri === this._selectedZoneIdx;
      ...
      const furnitureItems = this._mode === "edit" && this._editTool === "furniture" ? this._editFurniture : camera.attributes.furniture || [];
      for (let fi = 0; fi < furnitureItems.length; fi++) {
        const item = furnitureItems[fi];
        const isSelected = this._mode === "edit" && this._editTool === "furniture" && fi === this._selectedFurnitureIdx;
    ...
    _getEditZonesForTool() {
      switch (this._editTool) {
        case "no_go":
          return this._editNoGoZones;
        case "carpet":
          return this._editCarpetZones;
        case "low_clearance":
          return this._editLowClearanceZones;
        case "ramp":
          return this._editRamps;
        default:
          return this._editNoGoZones;
      }
    }
    _getEditLinesForTool() {
      switch (this._editTool) {
        case "wall":
          return this._editVirtualWalls;
        case "threshold":
          return this._editPassableThresholds;
        case "impassable":
          return this._editImpassableThresholds;
        case "cliff":
          return this._editCliffs;
        default:
          return this._editVirtualWalls;
      }
    }
    ...
    _renderFurniturePicker() {
      ...
      const buttons = Object.entries(types).map(
        ([id, info]) => `<button class="furniture-type-btn" data-type="${id}">${info.name}</button>`
      ).join("");
      return `
    ...
    _updateRoomList(card) {
      ...
      if (this._mode === "edit") {
        ...
        ].filter(([c]) => c > 0).map(([c, l]) => `${c} ${l}`).join(" \xB7 ");
    ...
    `;
      if (this._roomView === "list") {
        const listItems = roomEntries.sort(([idA, a], [idB, b]) => this._getRoomName(idA, a).localeCompare(this._getRoomName(idB, b))).map(([segId, room]) => {
          const sid = parseInt(segId, 10);
          const checked = this._selectedRooms.has(sid);
          const [r, g, b] = room.color || [135, 206, 235];
          const orderIdx = this._roomOrder.indexOf(sid);
          const orderLabel = checked && orderIdx >= 0 ? `<span class="room-order" style="background:rgb(${r},${g},${b})">${orderIdx + 1}</span>` : "";
          return `
        ...
        const selectedNames = roomEntries.filter(([segId]) => this._selectedRooms.has(parseInt(segId, 10))).map(([segId, room]) => this._getRoomName(segId, room));
        const selText = selectedNames.length > 0 ? selectedNames.join(", ") : "Tap rooms on the map to select";
        container.innerHTML = `
        ${viewToggle}
    ...
    async _saveMapEdits() {
      ...
      const buildZonePayload = /* @__PURE__ */ __name((zones) => zones.map((zone, i) => ({
        id: i + 1,
        type: zone.type ?? 0,
        hide: zone.hide ?? 0,
        roi: zone.roi
      ...
      try {
        ...
        this._exitEditMode();
        this._updateContent();
        const preSaveTs = this._hass?.states?.[this._entities.map]?.last_updated || "";
    ...
    _updateSettingsPanel(card) {
      ...
      const consumableHtml = availableConsumables.map((c) => {
